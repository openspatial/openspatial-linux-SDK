...
int is_ring_service_uuid(uuid128_t *a);
int is_ring_service_uuid16(uint16_t *a);

uint8_t nod_ring_parse_adv_record(const uint8_t *scan_data, const size_t scan_data_len, nod_ring_adv_t *ring_adv)
{
    // no data to parse
    if (scan_data == NULL)
        return 1;
    ...
    while (i < scan_data_len) {
        ...
        switch(type) {
            ...
            case ADV_TYPE_FLAGS:
                if (ring_adv != NULL)
                    memmove(&ring_adv->flags, data, data_len);
                break;
            case ADV_TYPE_UUID_128:
            {
                if ((data_len == sizeof(uuid128_t)) &&
                    !is_ring_service_uuid((uuid128_t*)data) )
                    return 2;

                if (ring_adv != NULL) {
                    memmove(&ring_adv->service_id, data, data_len);
                } else {
                    return 0;
                }
                break;
            }
            case ADV_TYPE_UUID_16:
            {
                if ((data_len == sizeof(uint16_t)) &&
                    !is_ring_service_uuid16((uint16_t *)data) )
                    return 2;
            ...
            case ADV_TYPE_LOCAL_NAME:
                if (ring_adv != NULL) {
                    nod_ring_parse_local_name(data, data_len, &ring_adv->local_name);
                }
                break;
            case ADV_TYPE_TX_POWER:
                if (ring_adv != NULL)
                    memmove(&ring_adv->tx_power, data, data_len);
                break;
            case ADV_TYPE_MANUFACTURER_DATA:
                if (ring_adv != NULL) {
                    nod_ring_parse_mfg_data(data, data_len, &ring_adv->mfg_data);
                } 
                break;
            default:
                return 3;
    ...
    return ((ring_adv != NULL) && is_ring_service_uuid(&ring_adv->service_id)) ? 0 : 2;
}

uint8_t nod_ring_adv_record_has_nod_uuid(const uint8_t *scan_data, const size_t scan_data_len)
{
    return (nod_ring_parse_adv_record(scan_data, scan_data_len, NULL) == 0);
}

int is_ring_service_uuid(uuid128_t *a)
{
    static const uint8_t RING_SERVICE_UUID_BYTES[16] = OS_SERVICE_UUID_LE;
    return (uuid128_cmp(a, (uuid128_t *)RING_SERVICE_UUID_BYTES) == 0); 
}

int is_ring_service_uuid16(uint16_t *a)
{
    static const uint8_t RING_SERVICE_UUID_BYTES[2] = HID_SERVICE_UUID_LE;
    return (uuid16_cmp(a, (uint16_t *)RING_SERVICE_UUID_BYTES) == 0); 
}

uint8_t nod_ring_parse_mfg_data(const uint8_t *bytes, uint8_t len, nod_ring_adv_mfg_t *mfg_data)
{
...
uint8_t nod_ring_parse_local_name(const uint8_t *bytes, uint8_t len, nod_ring_adv_info_t *local_name)
{
    if (len == 0)   return 1;
