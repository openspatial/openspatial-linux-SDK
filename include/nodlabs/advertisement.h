 ...
 */
typedef struct nod_ring_adv_state {
  uint8_t _reserved:4;
 ...
 */
typedef struct nod_ring_adv_info {
    nod_ring_adv_state_t state;
    uint16_t            version;
    uint8_t             _reserved[5];
 ...
 */
typedef struct nod_ring_adv_mfg {
    uint32_t    identifier;
    uint8_t     model_id;
    uint8_t     _reserved;
    uint16_t    product_id;
 ...
 */
typedef struct nod_ring_adv {
    uint8_t                     flags;
    uint8_t                     tx_power;
    nod_ring_adv_mfg_t      mfg_data;
    nod_ring_adv_info_t     local_name;
 ...
 */
uint8_t nod_ring_adv_record_has_anki_uuid(const uint8_t *scan_data, const size_t scan_data_len);
 ...
 * @param scan_data Bytes obtained by scanning vehicle advertising packets.
 * @param scan_data_len Length of bytes in scan_data.
 * @param nod_ring_adv Pointer to a structure for storing accumulated information for a vehicle.
 ...
 *
 * @discuss Pass NULL for nod_ring_adv in order to determine if this is an appropriate record.
 *
 * 
 */
uint8_t nod_ring_parse_adv_record(const uint8_t *scan_data, const size_t scan_data_len, nod_ring_adv_t *nod_ring_adv);
